...
class IrUiMenu(models.Model):
    ...
    def search_fetch(self, domain, field_names, offset=0, limit=None, order=None):
        if self.env.user == self.env.ref('base.user_root'):
            return super(IrUiMenu, self).search_fetch(domain, field_names, order=order)
        else:
            menus = super(IrUiMenu, self).search_fetch(domain, field_names, order=order)
            if menus:
                ...
                if limit:
                    menus = menus[:limit]
            return len(menus) if offset else menus
